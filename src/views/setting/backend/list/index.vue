...
import { fetchBackendList, fetchDeleteBackend } from '@/api/backend';
...
const createColumns = (): DataTableColumns<IBackend> => {
  const action: TableColumn<IBackend> = {
    title: '操作',
    key: 'actions',
    width: 200,
    align: 'center',
          ...
          h(
            NPopconfirm,
            {
              ...
              'on-positive-click': async () => {
                await fetchDeleteBackend(row.id!);
...
const ajaxFetchList = async (args) => {
  try {
    ...
    const res: any = await fetchBackendList(args);
    if (res.code === 200) {
